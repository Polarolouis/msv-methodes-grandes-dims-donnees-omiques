...
pca = PCA(table2[,-c(1,2)], scale.unit=TRUE, ncp=5)
```
...
barplot(pca$eig[,1]/sum(pca$eig[,1]), ylim = c(0,1))
```
...
\begin{columns}
  \begin{column}{0.6\textwidth}
      ...
      \small Avec $\bm{1}_{A:B, i} = 1$ ou $0$ si la $i^e$ observation n'est pas 
...
\vdots & \cdots & \vdots \\
Y_{n,1} & \cdots & Y_{n,q}\\
\end{pmatrix}}_{q = `r q`} \right. {\scriptstyle n = `r n`}$$
...
```{r graphique_simu_T1, echo=FALSE, fig.height=4.5, fig.cap="\\emph{p-value} pour le test basé sur la statistique T1"}
ggplot(result) +
...
```{r graphique_simu_T2, echo = FALSE, fig.height=4.5, fig.cap="\\emph{p-value} pour le test basé sur la statistique T2"}
ggplot(result) +
